...
def test_search_agent(mock_get_llm):
    ...
    with patch('agents.search_agent.DuckDuckGoSearchRun') as mock_search:
        ...
        mock_tool.run.assert_called_once_with("test query", max_results=config.get("search_results_limit", 5))
...
def test_web_fetcher_valid_url():
    ...
    # Assert for non-empty content and presence of HTML indicators (httpbin returns basic HTML)
    assert len(result) > 0
...
def test_reflection_loop_max_iterations(mock_graph, mock_prompt, mock_get_llm):
    ...
    # Simulate ainvoke with a simple state (non-async for unit test; in practice, use asyncio for full e2e)
    ...
    mock_astream.__aiter__ = lambda: [MagicMock(event="on_chat_model_stream", data={"chunk": MagicMock(content="token")})]
    ...
    assert any("on_chat_model_stream" in e.get("event", "") for e in event_list)  # Streaming event present
    
    # Verify max iterations logic indirectly: graph conditional edges should be set
